...
function generateIncrementalId() {
    let useridplayer = document.createElement('p'); //создаем еще параграф 
    useridplayer.innerText = `${tg.initDataUnsafe.user.id}`; //показываем user_id
    useridplayer.appendChild(useridplayer); //добавляем
}

// Использование:
const userId = generateIncrementalId();
