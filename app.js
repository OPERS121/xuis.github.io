...
let afkclaim = (localStorage.getItem('afkclaim')) ? parseInt(localStorage.getItem('afkclaim')) : 0;
...
let factoregg7 = 1;
 ...
 // Массив игроков
 const players = [
	{ name: "Игрок 1", counter: 1000 },
	{ name: "Игрок 2", counter: 1500 },
	{ name: "Игрок 3", counter: 800 },
	{ name: "Игрок 4", counter: 2000 },
	{ name: "Игрок 5", counter: 1200 }
];

// Сортировка игроков по балансу (по убыванию)
players.sort((a, b) => b.counter - a.counter);
...
players.forEach(player => {
	...
	balanceCell.textContent = player.counter;
});
...
setTimeout(() => {
	...
	document.querySelector("#afkclaim").textContent = ("AFK CLAIM: " + formatter(afkclaim));
...
let opentopplayers = document.getElementById('opentopplayers');
...
function intialise()
{
  ...
  var countdown = setInterval(function() {
    ...
    if (remain < 0) {
      ...
      document.getElementById("farming").innerHTML = "+150 $COIN";
...
openModalShop.addEventListener('click', () => {
	...
	if (localStorage.getItem('chiken') == 1) {
		...
		buychiken6yes.style.display = 'none';
		buychiken7yes.style.display = 'none';
		var lastchosen = onechiken
		var lastchosendown = onechikendown
	}
	if (localStorage.getItem('chiken') == 2) {
		buychiken1.style.display = 'none';
		buychiken1yes.style.display = 'block';
		...
		buychiken6yes.style.display = 'none';
		buychiken7yes.style.display = 'none';
		var lastchosen = twochiken
		var lastchosendown = twochikendown
	}
	if (localStorage.getItem('chiken') == 3) {
		buychiken1.style.display = 'none';
		buychiken1yes.style.display = 'block';
		...
		buychiken6yes.style.display = 'none';
		buychiken7yes.style.display = 'none';
		var lastchosen = freechiken
		var lastchosendown = freechikendown
	}
	if (localStorage.getItem('chiken') == 4) {
		buychiken1.style.display = 'none';
		buychiken1yes.style.display = 'block';
		...
		buychiken6yes.style.display = 'none';
		buychiken7yes.style.display = 'none';
		var lastchosen = fourchiken
		var lastchosendown = fourchikendown
	}
	if (localStorage.getItem('chiken') == 5) {
		buychiken1.style.display = 'none';
		buychiken1yes.style.display = 'block';
		...
		buychiken6yes.style.display = 'none';
		buychiken7yes.style.display = 'none';
		var lastchosen = fivechiken
		var lastchosendown = fivechikendown
	}
	if (localStorage.getItem('chiken') == 6) {
		buychiken1.style.display = 'none';
		buychiken1yes.style.display = 'block';
	...
	if (localStorage.getItem('chiken') == 7) {
		...
		var lastchosen = rabbit
		var lastchosendown = rabbitdown
...
function saveMain() {
	...
	localStorage.setItem('afkclaim', parseInt(afkclaim));
...
var upper = document.getElementById('clickcontainer');

var lastchosen = onechiken
var lastchosendown = onechikendown

onechiken.addEventListener('click', mousecl);

function mousecl(event) {
	lastchosen.style.display = 'none';
	lastchosendown.style.display = 'block';
	
	lastchosen.setAttribute('draggable',false);
	lastchosendown.setAttribute('draggable',false);
	
	
	setTimeout(() => {
		lastchosen.style.display = 'block';
		lastchosendown.style.display = 'none';
	...
	var cchj = clickchikentext1
	
	if (lastchosen.id == "fourchiken"){
		clickchikentext1 = document.getElementById('clickchikentext4');
	}
	if (lastchosen.id == "fivechiken"){
		clickchikentext1 = document.getElementById('clickchikentext5');
	}
	if (lastchosen.id == "duck"){
		clickchikentext1 = document.getElementById('clickchikentext6');
	}
	if (lastchosen.id == "rabbit"){
		clickchikentext1 = document.getElementById('clickchikentext7');
	}
	
	var xxhg = clickchikentext1.cloneNode(true);
		xxhg.style.display = 'block';
		xxhg.style.zIndex=100;
		xxhg.style.position = "absolute";
		xxhg.style.left = event.clientX+'px';
		xxhg.style.top = (event.clientY-100)+'px';
		xxhg.style["pointer-events"] = "none";
	...
	setTimeout(() => {
		...
		upper.appendChild(xxhg);
		setTimeout(() => {
		document.getElementById(iddx).remove();
	}, 600);
	}, 100);
}
...
function start() {
  ...
  console.log("d")
	...
	console.log("e")
    item = list.querySelectorAll('li')[15]
    console.log("f")
...
opentopplayers.addEventListener('click', () => {
	chikenss.style.display = 'none';
