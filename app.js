...
if (getcookie("stoptime") != false){
	
	if (getcookie("stoptime") != "NaN" && getcookie("starttime") != "NaN"){
	//даные с печени
	stop_time = new Date(getcookie("stoptime"));
	start_time = new Date(getcookie("starttime"));
...
function startf() {
  ...
  start_time = new Date(); 
  document.cookie = "starttime="+start_time.toString()
  // получаем время окончания таймера
  stop_time = start_time; 
  stop_time.setMinutes(stop_time.getMinutes() + count); 

	document.cookie = "stoptime="+stop_time.toString()
...
setTimeout(() => {
	...
	if (remain < 0 && started == true) {
	  ...
	  // куки под 0
    document.cookie = "starttime=NaN"

	document.cookie = "stoptime=NaN"
	}
}, 150);
});
